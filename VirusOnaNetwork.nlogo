globals [
  cumulative-ward       ;; Totale cumulativo passati in degenza
  cumulative-icu        ;; Totale cumulativo passati in TI
  deaths-ward           ;; Morti totali in degenza
  deaths-icu            ;; Morti totali in TI
  total-ward-days       ;; Somma totale dei giorni passati in degenza da tutti
  total-icu-days        ;; Somma totale dei giorni passati in TI da tutti
  ward-history          ;; Vettore per memorizzare la degenza ordinaria nel tempo
  icu-history           ;; Vettore per memorizzare la terapia intensiva nel tempo
]

turtles-own
[
  infected?           ;; se true, il nodo è infetto
  resistant?          ;; se true, il nodo è immune/guarito
  dead?               ;; se true, il nodo è morto ma resta visibile
  hospitalized?       ;; se true, il nodo è in degenza ordinaria
  icu?                ;; se true, il nodo è in terapia intensiva
  virus-check-timer   ;; contatore dei tick per chi è a casa
]

to setup
  clear-all
  
  set cumulative-ward 0
  set cumulative-icu 0
  set deaths-ward 0
  set deaths-icu 0
  set total-ward-days 0
  set total-icu-days 0
  set ward-history []
  set icu-history []
...
to go
  ...
  ;; Avanzamento del timer per i nodi vivi
  ...
  ;; Accumulo dei giorni totali per il calcolo delle degenze medie
  set total-ward-days total-ward-days + count turtles with [hospitalized?]
  set total-icu-days total-icu-days + count turtles with [icu?]
  
  ;; Salvataggio dei valori attuali nei vettori storici per Python
  set ward-history lput (count turtles with [hospitalized?]) ward-history
  set icu-history lput (count turtles with [icu?]) icu-history
  
  ;; Dinamiche ospedaliere giornaliere (a ogni tick)
  do-hospital-dynamics 
  
  ;; Risoluzione clinica a domicilio (al ritmo del timer)
  do-virus-checks
...
to become-hospitalized 
  set hospitalized? true
  set icu? false
  set color orange
  set cumulative-ward cumulative-ward + 1
end
...
to become-icu 
  ...
  set icu? true
  set color magenta
  set cumulative-icu cumulative-icu + 1
end

to spread-virus
  ;; Chi è ricoverato è isolato. Solo i vicini sani (non infetti, non immuni, non morti) possono contagiarsi
  ask turtles with [infected? and not dead? and not hospitalized? and not icu?]
    [ ask link-neighbors with [not infected? and not resistant? and not dead?]
        [ if random-float 100 < virus-spread-chance
            [ become-infected ] ] ]
end

to do-hospital-dynamics
  ask turtles with [infected? and not dead?]
  [
    ifelse icu?
    [
      ;; Controllo giornaliero Terapia Intensiva (degenza media: 10 giorni)
      if random-float 100 < 10.0 [
        ifelse random-float 100 < 50.0 
          [ 
            set deaths-icu deaths-icu + 1
            become-dead 
          ]
          [ recover-logic ]
      ]
    ...
    [
      ifelse hospitalized?
      [
        ;; Controllo giornaliero Degenza Ordinaria (degenza media: 14 giorni)
        if random-float 100 < 7.14 [
          ifelse random-float 100 < 20.0 
            [ 
              set deaths-ward deaths-ward + 1
              become-dead 
            ]
            [ recover-logic ]
        ]
      ...
      [
        ;; Rischio giornaliero di aggravamento per chi è a casa
        let daily-icu-chance (recovery-chance * 0.0109) / virus-check-frequency
        let daily-ward-chance (recovery-chance * 0.0819) / virus-check-frequency
        
        let admission-roll random-float 100
        
        ifelse admission-roll < daily-icu-chance
        [ become-icu ]
        [
          if admission-roll < (daily-icu-chance + daily-ward-chance)
          [ become-hospitalized ]
        ]
      ]
    ]
...
to do-virus-checks
  ;; Guarigione a casa al ritmo del timer (nessun decesso a domicilio)
  ask turtles with [infected? and virus-check-timer = 0 and not dead? and not icu? and not hospitalized?]
  [
...
to export-data
  ...
  file-open "dati_ospedale.csv"
  file-print "Tick,Ospedalizzati,Terapia_Intensiva"
  
  let i 0
  while [i < length ward-history] [
    file-print (word i "," (item i ward-history) "," (item i icu-history))
    set i i + 1
  ]
...
"" ""
PENS
"hospitalized" 1.0 0 -2674135 true "" "plot (count turtles with [hospitalized?]) / (count turtles) * 1000"
"icu" 1.0 0 -2674135 true "" "plot (count turtles with [icu?]) / (count turtles) * 1000"      
